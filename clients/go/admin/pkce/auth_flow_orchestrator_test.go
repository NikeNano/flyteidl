...
func TestFetchFromCache(t *testing.T) {
	...
	orchestrator, err := NewTokenOrchestrator(ctx, Config{}, &TokenCacheInMemoryProvider{}, mockAuthClient, []string{})
...
func TestFetchFromAuthFlow(t *testing.T) {
	...
	t.Run("fetch from auth flow", func(t *testing.T) {
		...
		orchestrator, err := NewTokenOrchestrator(ctx, Config{}, tokenCache, mockAuthClient, []string{})
		assert.NoError(t, err)
